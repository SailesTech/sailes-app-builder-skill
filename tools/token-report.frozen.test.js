 ...
 *
 * ---------------------------------------------------------------------------------------------
 * PINNED `--json` OUTPUT SHAPE — this is this suite's contract, not the spec's. The spec names
 * the categories a report must cover (lead vs subagent totals, turns, first-turn context, peak,
 * top-10% share, per-role distribution, unprefixed non-built-in spawns by name) but never a JSON
 * schema. Absent a named shape, this suite pins one — the same move already made for Q7's
 * `malformedLines` field in the frozen plan — so the implementer has a concrete contract to meet.
 * A mismatch here is a legitimate red test and a naming disagreement to raise with the lead, not
 * something to silently reinterpret in either direction.
 *
 *   {
 *     "lead":     { "sessionCount", "totalContextTokens",
 *                   "turns": { "p50", "max" }, "firstTurnContext": { "p50" },
 *                   "peak": { "p50", "max" }, "top10PercentShare" },
 *     "subagent": { ...same shape as lead...,
 *                   "perRole": { "<bare-role-name>": { "sessionCount", "totalContextTokens" },
 *                                "unknown": { "sessionCount", "totalContextTokens" } } },
 *     "spawns":   { "<unprefixed-non-built-in-name>": <count>, ... },
 *     "malformedLines": <int>
 *   }
 *
 ...
 * bucketing; missing/unparsable `.meta.json` -> bucket `unknown`.
...
function sumPerRole(perRole) {
  return Object.values(perRole || {}).reduce((acc, r) => acc + (r.totalContextTokens || 0), 0);
}
...
test('P0-01 — empty transcript directory reports all-zero, no crash, no NaN/Infinity', () => {
  ...
  try {
    ...
    assert.strictEqual(json.lead.sessionCount, 0);
    assert.strictEqual(json.subagent.sessionCount, 0);
    assertFinite(json.lead.totalContextTokens, 'lead.totalContextTokens');
    assert.strictEqual(json.lead.totalContextTokens, 0);
    assertFinite(json.subagent.totalContextTokens, 'subagent.totalContextTokens');
    assert.strictEqual(json.subagent.totalContextTokens, 0);
...
test('P0-04 — lead session with no subagents/ directory at all does not crash', () => {
  ...
  try {
    ...
    assert.strictEqual(json.lead.totalContextTokens, 2500);
    assert.strictEqual(json.subagent.sessionCount, 0);
    assert.strictEqual(json.subagent.totalContextTokens, 0);
  } finally {
    rm(dir);
...
test('P0-05 — orphaned subagent group (no parent <session>.jsonl) still counts, no crash', () => {
  ...
  try {
    ...
    assert.strictEqual(status, 0);
    assert.strictEqual(json.lead.sessionCount, 0);
    assert.strictEqual(json.subagent.sessionCount, 1);
    assert.strictEqual(json.subagent.totalContextTokens, 1800);
  } finally {
    rm(dir);
  }
});
...
test('P0-06 — split message (usage on line 1, tool_use on a duplicate message.id line 2): usage once, spawn counted', () => {
  ...
  try {
    const { status, json } = runJson(dir);
    assert.strictEqual(status, 0);
    assert.strictEqual(json.lead.totalContextTokens, 9000, 'usage must be counted once, not skipped, not doubled');
    assert.strictEqual(json.spawns['be-dev'], 1, 'tool_use on the duplicate-id line must still be collected (2026-09-12 regression: 10 vs 177)');
  } finally {
    rm(dir);
  }
});
...
test('P0-07 — repeated usage on the same message.id counted once, not twice', () => {
  ...
  try {
    const { status, json } = runJson(dir);
    assert.strictEqual(status, 0);
    assert.strictEqual(json.lead.totalContextTokens, 6000);
  } finally {
    rm(dir);
  }
});
...
test('P0-08 — two distinct message.ids with distinct usage are both counted', () => {
  ...
  try {
    const { status, json } = runJson(dir);
    assert.strictEqual(status, 0);
    assert.strictEqual(json.lead.totalContextTokens, 30000);
...
test('P0-09 — an assistant message with no usage field contributes 0, not NaN, still counts as a turn', () => {
  ...
  try {
    const { status, json } = runJson(dir);
    assert.strictEqual(status, 0);
    assert.strictEqual(json.lead.totalContextTokens, 5000);
    assert.strictEqual(json.lead.turns.max, 2);
  } finally {
    rm(dir);
  }
...
test('P0-10 — a session where every assistant line lacks usage: zero totals, no NaN/Infinity', () => {
  ...
  try {
    ...
    assert.strictEqual(json.lead.totalContextTokens, 0);
    assert.strictEqual(json.lead.turns.max, 2);
    assertFinite(json.lead.peak.max, 'lead.peak.max');
    assert.strictEqual(json.lead.peak.max, 0);
...
test('P0-11 — an assistant message with empty tool_use contributes 0 spawns but still counts usage', () => {
  ...
  try {
    ...
    assert.strictEqual(status, 0);
    assert.strictEqual(json.lead.totalContextTokens, 5000);
    assert.strictEqual(Object.keys(json.spawns || {}).length, 0);
  } finally {
    rm(dir);
  }
});
...
test('P0-12 — usage with only input present (no cache fields) treats missing cache fields as 0', () => {
  ...
  try {
    const { status, json } = runJson(dir);
    assert.strictEqual(status, 0);
    assert.strictEqual(json.lead.totalContextTokens, 7000);
  } finally {
    rm(dir);
  }
});
...
test('P0-14 — lead and subagent totals are separate buckets, never summed together', () => {
  ...
  try {
    const { status, json } = runJson(dir);
    assert.strictEqual(status, 0);
    assert.strictEqual(json.lead.totalContextTokens, 15000);
    assert.strictEqual(json.subagent.totalContextTokens, 5000);
  } finally {
    rm(dir);
  }
});
...
test('P0-15 — multiple lead sessions and subagents across them sum correctly per bucket', () => {
  ...
  try {
    const { status, json } = runJson(dir);
    assert.strictEqual(status, 0);
    assert.strictEqual(json.lead.totalContextTokens, 3000);
    assert.strictEqual(json.subagent.totalContextTokens, 600);
    assert.strictEqual(json.lead.sessionCount, 2);
  } finally {
    rm(dir);
  }
});
...
test('P0-18 — peak is the max single call in a session, not cumulative, not last', () => {
  ...
  try {
    const { status, json } = runJson(dir);
    assert.strictEqual(status, 0);
    assert.strictEqual(json.lead.peak.max, 50000);
  } finally {
    rm(dir);
  }
});
...
test('P0-22 — per-role distribution buckets by sibling .meta.json agentType, prefix stripped', () => {
  ...
  try {
    const { status, json } = runJson(dir);
    assert.strictEqual(status, 0);
    assert.ok(json.subagent.perRole, 'expected subagent.perRole');
    assert.strictEqual(json.subagent.perRole['be-dev'].totalContextTokens, 4000);
    assert.strictEqual(json.subagent.perRole['qa'].totalContextTokens, 6000, 'sailes-app-builder:qa must bucket under bare "qa"');
    assert.strictEqual(json.subagent.totalContextTokens, 10000);
  } finally {
    rm(dir);
  }
});
...
test('P0-23 — a subagent transcript with no sibling .meta.json falls into an explicit unknown bucket', () => {
  ...
  try {
    const { status, json } = runJson(dir);
    assert.strictEqual(status, 0);
    assert.ok(json.subagent.perRole && json.subagent.perRole.unknown, 'expected an explicit unknown bucket');
    assert.strictEqual(json.subagent.perRole.unknown.totalContextTokens, 1234);
    assert.strictEqual(sumPerRole(json.subagent.perRole), json.subagent.totalContextTokens, 'per-role buckets + unknown must reconcile to the overall subagent total — a dropped transcript would under-count here');
  } finally {
    rm(dir);
  }
});
...
test('P0-24 — an unprefixed Task subagent_type is tallied under its bare name', () => {
  ...
  try {
    const { status, json } = runJson(dir);
    assert.strictEqual(status, 0);
    assert.strictEqual(json.spawns['be-dev'], 1);
  } finally {
    rm(dir);
  }
});
...
test('P0-25 — a sailes-app-builder:-prefixed Task subagent_type is excluded from the unprefixed tally', () => {
  ...
  try {
    const { status, json } = runJson(dir);
    assert.strictEqual(status, 0);
    assert.ok(!('be-dev' in json.spawns), 'prefixed spawn must not leak into the bare-name bucket');
    assert.ok(!('sailes-app-builder:be-dev' in json.spawns), 'prefixed spawns are excluded from this tally entirely');
  } finally {
    rm(dir);
  }
});
...
test('P0-26 — "general-purpose" (built-in) is excluded from the unprefixed non-built-in tally', () => {
  ...
  try {
    const { status, json } = runJson(dir);
    assert.strictEqual(status, 0);
    assert.ok(!('general-purpose' in json.spawns));
  } finally {
    rm(dir);
  }
});
...
test('P0-26b — the full Q2 built-in exclusion list (Explore, Plan, claude, statusline-setup, claude-code-guide) is excluded', () => {
  ...
  try {
    ...
    for (const name of ['Explore', 'Plan', 'claude', 'statusline-setup', 'claude-code-guide']) {
      assert.ok(!(name in json.spawns), `built-in "${name}" must not appear in the unprefixed tally`);
    }
    assert.strictEqual(Object.keys(json.spawns || {}).length, 0, 'all six built-ins excluded -> tally must be empty');
  } finally {
    rm(dir);
  }
});
...
test('P0-27 — multiple unprefixed spawns are tallied per distinct name', () => {
  ...
  try {
    const { status, json } = runJson(dir);
    assert.strictEqual(status, 0);
    assert.strictEqual(json.spawns['be-dev'], 3);
    assert.strictEqual(json.spawns['explorer'], 2);
  } finally {
    rm(dir);
  }
});
...
test('P0-28 — a Task tool_use on a duplicate-message.id line is still counted (spawn-side regression proof)', () => {
  ...
  try {
    const { status, json } = runJson(dir);
    assert.strictEqual(status, 0);
    assert.strictEqual(json.spawns['qa'], 1);
  } finally {
    rm(dir);
  }
});
...
test('P0-29 — --since/--until window includes only the session inside [since 00:00, until 00:00) local', () => {
  ...
  try {
    ...
    assert.strictEqual(status, 0);
    assert.strictEqual(json.lead.sessionCount, 1);
    assert.strictEqual(json.lead.totalContextTokens, 222);
  } finally {
    rm(dir);
  }
});
...
test('P0-30 — --since is inclusive at D 00:00 local, --until is exclusive at D 00:00 local (asymmetric boundaries)', () => {
  ...
  try {
    ...
    const { status, json } = runJson(dir, ['--since', '2026-09-11', '--until', '2026-09-13']);
    assert.strictEqual(status, 0);
    assert.strictEqual(json.lead.sessionCount, 1, 'exactly one of the two boundary sessions must be included');
    assert.strictEqual(json.lead.totalContextTokens, 555, 'the --since-boundary session (included) must be the one counted, not the --until-boundary one (excluded)');
  } finally {
    rm(dir);
  }
});
...
test('P0-31 — no --since/--until given: all sessions included regardless of mtime', () => {
  ...
  try {
    ...
    assert.strictEqual(status, 0);
    assert.strictEqual(json.lead.sessionCount, 2);
    assert.strictEqual(json.lead.totalContextTokens, 30);
  } finally {
    rm(dir);
  }
});
...
test('P0-32 — --since alone (no --until) is an open-ended upper bound', () => {
  ...
  try {
    ...
    assert.strictEqual(status, 0);
    assert.strictEqual(json.lead.sessionCount, 1);
    assert.strictEqual(json.lead.totalContextTokens, 60);
  } finally {
    rm(dir);
  }
});
...
test('P0-34 — default text output and --json output agree on the headline totals', () => {
  ...
  try {
    ...
    assert.strictEqual(textStatus, 0);
    assert.strictEqual(jsonStatus, 0);
    assert.ok(text.includes(String(json.lead.totalContextTokens)), `default text output must surface the same lead total (${json.lead.totalContextTokens}) as --json:\n${text}`);
    assert.ok(text.includes(String(json.subagent.totalContextTokens)), `default text output must surface the same subagent total (${json.subagent.totalContextTokens}) as --json:\n${text}`);
  } finally {
    rm(dir);
  }
});
...
test('P0-35 — a malformed JSON line is skipped and counted, other lines still processed, exit code unaffected', () => {
  ...
  try {
    ...
    assert.strictEqual(json.lead.totalContextTokens, 8000, 'both valid lines (5000 + 3000) must still be counted');
