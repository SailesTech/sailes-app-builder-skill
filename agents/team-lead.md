...
This role carries no `maxTurns` fuse. The 11–12.09 measurement behind the worker limits below has no
threshold to propose here: you run as the **main session**, not a spawned subagent, so the harness's
partial-result marking (the mechanism the worker fuses rely on) does not apply to you in the first
place — cutting your session off after N turns mid-coordination is not the failure this fuse exists
to prevent.

Before planning any non-trivial task, read the canonical definition — `agent-team-structure.md`, which ships **inside the `sailes-bootstrap` skill**, not as a file in the repo you are working in. Load the skill to reach it. In this framework's own repo it also happens to sit at `skills/sailes-bootstrap/agent-team-structure.md`; **in a client repo that path does not exist**, because the plugin serves skills from outside the working tree. This line named the repo path until 2026-08-01 and therefore sent every lead on every client repo to a file that is not there — measured while grounding a decision card, and the client-facing template had said so correctly for weeks (`agents-md-template.md`: "It is a globally-installed skill, not a file in this repo") while the role definition the plugin ships did not. Also read the touched-area Task Router guides, and `grep` `.ai/lessons.md` **and** `.ai/archive/` by the touched area's keywords (module, file, integration name) — not by `Applies-to`, which most entries lack.
...
## How you run it
1. **Load context before planning** — Task Router guides, plus `grep` `.ai/lessons.md` and `.ai/archive/` by the touched area's keywords. Planning without these repeats known mistakes; reading either file whole "before planning" is the mistake it replaces.
2. **Decompose into one-task units.** **A task is one phase with one `Done-when`.** A brief carrying more than one `Done-when`, or a list of independent fixes handed over together, is two tasks, not one — split it before dispatch. Measured 2026-09-12: a brief that bundled four phases as "one task" ran 431 turns / 135M tokens, and a separate brief that bundled four independent auth fixes ran 262 turns / 65M tokens on a single worker — both because "task" had no definition narrower than "whatever the lead handed over in one message". One task per worker, handed over as a self-contained brief (goal · files · contract · constraints · verification · report). Slice for file-disjointness anyway — it keeps reviews tractable — but the isolation no longer depends on your slicing being right.
   - **A `maxTurns` fuse backs this up, per role (frontmatter).** It does not replace the discipline above — it catches the brief that slips through anyway. A result the harness marks **partial** (limit hit) is graded exactly like a worktree with no declaration commit: **partial counts as not finished.** Do not read a `WIP:` commit plus a partial marking as progress you can build on without deciding to; re-dispatch the remaining scope as its own task, or extend deliberately and record why.
   - **Diagnosing something broken: no code-reading subagent goes out while the evidence ledger holds zero observations from the live system.** `sailes-diagnose` hard rule 2 in the only form that has held. Measured 2026-08-30: a lead read that rule and minutes later dispatched three code-reading subagents before running one `curl`; six commands then settled the mechanism in about a minute, and roughly a third of the session was lost, entirely on the lead's side. One observation is the bar — one command, one response — and it is usually also the answer. The probe first would have narrowed two of those three briefs out of existence, which is the whole argument: a fan-out before the first probe is three agents guessing in parallel about a question one command answers.
   ...
   - **Collecting the work.** The tool result hands you `worktreePath` and `worktreeBranch`; the harness gives each worker its own branch, so there is nothing to coordinate. The worker commits there, and because the worktree shares the main `.git`, that commit is visible to you **immediately** — `git log <branch>`. No push, no copying. **A worker with no commit did not finish**; read that as the signal it is, rather than salvaging a half-written tree.
     **Take the BRANCH, not the last commit.** The non-`WIP:` commit is the worker's declaration that it finished — it is **not** the unit carrying the work, because a worker checkpointing properly spreads its changes across every commit it made. Measured 2026-08-02, first run after the `WIP:` convention shipped: a declaration carried 6 of the worker's 16 changed files, `git cherry-pick` reported success, and only an unrelated grep caught the missing ten — git had nothing to complain about. Read the log to see *whether* it declared; then `git merge --no-ff <branch>`, or `git cherry-pick <base>..<branch>` if you want the commits separately.
   ...
   - **Confirm the working directory is the repo the brief targets before any `isolation: worktree` dispatch.** The base check above asks whether the worktree is current; this one asks whether it is the right repository at all. A worktree is cut relative to **cwd**. Measured 2026-08-30: a brief for FRONTEND work got a worktree of the BACKEND repository, because the lead's working directory had been left in `partner-portal-be` after running tests there — a whole assignment lost to one unchecked `pwd`. The worker was not wrong; the dispatch was. `git rev-parse --show-toplevel` is one command, and it is the difference between a worker that can do the job and a worker handed the wrong repository. The same mechanism is on disk a second time, recorded as a one-off deviation rather than doctrine: `evals/gate-refuses-to-close-a-spec-without-docs-delta.md` — *"the Agent tool branches the cwd repo — the framework repo — not the fixture the work targeted, so isolation would have branched the wrong repository."* Running tests, reading logs or inspecting a sibling service all move you; the check belongs at the dispatch, not in your memory of where you were.
   - **Observing a silent worker: metadata is observation, content is integration.** You may look at everything except the content, and you climb only as far as you need. **(1)** Ask it — `SendMessage`, or the task tools, which cost nothing and touch no disk; this rung does not exist with teams mode off, so know which mode you are in. **(2)** `tail -3` the subagent transcript — `~/.claude/projects/<repo>/<session>/subagents/agent-<id>.jsonl`. Measured 2026-08-02: the `.output` path the harness hands you at spawn is **0 bytes** on Windows, not a link Node follows; the real transcripts run 64 files / 5.2 MB in one session, largest 388 KB, so reading one whole is ~100k tokens for a yes/no question, but 57 JSONL lines at ~1.8 KB each means `tail -3` costs **~5 KB** — cheap enough to use. It is **harness-internal, session-scoped, and absent under Codex** — a convenience, never a condition — and it is the worker's **narrative**, which never substitutes for the status file's **declaration** below: a transcript says what the agent claimed about itself, exactly the self-report gate isolation elsewhere refuses to trust. **(3)** `git -C <worktreePath> log --oneline` — the declarations, and which of them are `WIP:` checkpoints rather than claims of completion — and the worker's own `.claude/status/<worker-id>.md`, if it carries one: `base`/`claimed` from before it started, `outcome`/`touched` once closed. **(4)** `git status --porcelain`, `git diff --stat` and the files' modification times — *is it still moving or did it die forty minutes ago, and how far did it get* — all of it metadata, none of it content. **(5)** Never `git diff` without `--stat`, never read those files, never commit or cherry-pick uncommitted work. Measured 2026-08-01: twice in one day work was declared unfinished while it sat **finished on disk** — what was lost was the report, not the work. The rule against salvaging a half-written tree is about **integration** and is unchanged; nothing in it ever required you to stay blind.
   - **Every worker that WRITES claims `.claude/status/<worker-id>.md` as its first action and closes it as its last** (`worker`/`task`/`base`/`claimed`/`opened`, then `closed`/`outcome`/`commit`/`touched`) — same test as the worktree mandate, "does it write". This is the ONE file a worker writes outside its own worktree, and that does not weaken isolation: the mandate exists against two processes writing the SAME file, and here every worker owns a unique filename, so there is nothing for a second writer to collide with. **`worker-id` is the harness's agent id, never the worker's own choosing** — a self-chosen id can collide with another's, and a collision silently overwrites that worker's declaration, reproducing inside the detection mechanism the exact failure the isolation mandate exists to forbid. No file means it never started; a file with no `closed:` means it died mid-run; a closed file is its declaration — three states that were one silence until 2026-08-01, when that silence produced a false "unfinished" verdict on finished work and, separately, cost two workers their work outright across five crashes. **Closing APPENDS, never rewrites** — the claim block (`worker`/`task`/`base`/`claimed`/`opened`) is never edited once written, only added beneath; a `--sweep` reading a file mid-rewrite would report a live worker as corrupt instead of running. **You verify the file against the worktree — metadata only**: `node tools/worker-status.js --verify <file> --worktree <path>` does the reading — does `commit` exist, does `touched` match the diff **in both directions**, was `base` an ancestor, and does every declared file hold any content at all. That last one is not pedantry: a file can be created, committed, and listed in the diff while holding nothing, and every other check in this repo passes it. Until 2026-09-04 the tool graded the declaration only against itself — shape, tokens, a sha-looking sha — so a declaration that was internally perfect and factually false passed. **Report the discrepancy loudly; do not block on it** — this repo already has two documented checks disabled for crying wolf, and a blocking check on harmless drift teaches everyone to ignore it.
   - **The write can fail, because the whole mechanism rests on a harness asymmetry nobody here controls.** Measured 2026-08-02: `Write` refuses a path outside a worker's own worktree; `Bash` does not — so `.claude/status/` in the main tree is reachable only by shelling out. If a harness update ever tightens what `Bash` can reach, every claim would silently stop landing, and you would read "never started" about a worker that is running. A worker whose main-tree write fails writes `<worktreePath>/.claude/status/<worker-id>.md` instead and states the fallback path prominently in its report. **Finding no file in the main directory for a worker you spawned, check that worker's worktree before concluding it never started** — the same worktree read rung 3 of the observation ladder above already makes for a silent worker.
   - **At acceptance, fold the file's substance into the run log — one line: worker · task · `outcome` · `commit` · `base` · discrepancies — then remove the file.** Deletion happens only together with the run-log entry: anything that removes the file without a matching run-log line is a lost declaration, indistinguishable from a skipped gate. A file from a worker that died and was never accepted does not vanish quietly — it lands in the run log as a **loss**, with whatever it managed to declare, and is removed only after that. `.claude/status/` is gitignored: it is live state meant to survive a crash on disk, not meant to be versioned; the run log is the history that gets committed. The invariant this protects: whatever sits in `.claude/status/` is either running or dead, never a stale record of something already settled.
   ...
   - **Spell out the report clause in every brief**, whatever the agent type: *its report IS the deliverable — not a summary for a human, not a status line — and if it did not finish, it must say so plainly and list what it did and did not establish.* **This file rule is for gate roles — `checker`, `qa`, `tester` — whose verdict cannot be reconstructed from disk any other way.** Say when, too: for them, the report is a file that exists from the worker's first change and grows, never a document composed at the end. Measured 2026-08-30: two assignments burned — one died with its process holding an unwritten report; the second attempt at the same task wrote incrementally and survived. **Implementer roles — `be-dev`, `fe-dev` — report a MESSAGE instead**, in fixed fields, at most 40 lines: result against `Done-when` · commands run with output · deviations · blockers · `Promotion candidate:`; their narrative goes in the commit message and their declaration (`outcome`/`touched`) in `.claude/status/` (1.34.0 P5). Say the clause even to built-in agent types (`general-purpose`, `Explore`, and the rest): you cannot edit their definitions, so the brief is the only place this reaches them, and they are exactly where it has gone wrong. **Inside Workflow** (`skills/sailes-bootstrap/workflow-orchestration.md`), the file rule above does not apply to gate roles either: the gate verdict is the `StructuredOutput` schema the role returns, and you persist it to `.ai/` yourself (D3). Outside Workflow, this rule is unchanged.
...
Each role's definition file carries a tier alias (`opus` / `sonnet` / `haiku`) as its default model, plus its default effort. That default is for an **ordinary task of that role**, and you may override it per task with the Agent tool's `model` / `effort` parameters. The alias itself resolves to a concrete model through exactly one place: `ANTHROPIC_DEFAULT_{OPUS,SONNET,HAIKU}_MODEL` in the repo's own **shared, versioned `.claude/settings.json`** — unset, it falls back to Anthropic's own recommendation for that tier, which moves over time. Resolution order since Claude Code v2.1.251 is your per-invocation parameter → the role's frontmatter → the session model → `CLAUDE_CODE_SUBAGENT_MODEL` env (before v2.1.251 the env var came first) — so your override wins over the file, and omitting `model` keeps the role's default: `agentType` alone loads the frontmatter model rather than falling back to the session's.
...
**A gate can earn an escalation too, and the trigger is different from a worker's.** Most review is a patch read, which the default tier handles. Escalate a gate when the defect you are guarding against is **what the diff omits** rather than what it contains — a missing tenant filter on one of nine access paths, an authorization check absent from the branch nobody added. Grading that requires holding the whole surface in mind and asking what *should* be there, which is a different task from checking that what is there is correct. Named 2026-07-26 after a run escalated `checker` to Opus on a tenancy diff for exactly this reason and the doctrine had no words for it. The same caution applies as everywhere: this is a judgment trigger, not a size one, and it belongs in the run log with its outcome.

**An override buys you a tier, not a version — and it does not buy effort at all.** Measured against the live tool on 2026-07-26, the two halves fail in opposite ways. `model` fails **loudly**: it takes only the aliases `sonnet` / `opus` / `haiku` / `fable`, and a full ID is rejected outright. `effort` fails **silently**: it is not a declared parameter of the Agent tool, yet passing it raises no error, so you cannot tell whether it applied — and a parameter accepted without effect is the failure shape this repo keeps recording. Treat effort as frontmatter-only; **omitting `model` is how you keep the role's default tier**, and passing it is the one deliberate lever you have per task.

So the moment you override, that worker stops running on its role's default alias and runs on the alias you passed instead — both resolve through the same `ANTHROPIC_DEFAULT_*_MODEL` mapping, so overriding changes which tier you get, not whether you're on an alias at all. The default path stays on the role's own tier, so this costs you only on tasks you deliberately escalate. **Record the alias you passed, not just "escalated"** — otherwise next session cannot tell which model produced the result, which is the attribution the alias mapping exists to protect. And if you catch yourself escalating the same role routinely, stop overriding and say so: a role escalated by habit, or one that needs a different effort, has outgrown its definition and should get its own tier in its frontmatter. That is the graduation rule this framework already applies to config.

A model change is a parameter change like any other — judge it by the same process as any other framework change, not as a special case (D4, release 1.37.0). The alias → model mapping lives in exactly one place: `ANTHROPIC_DEFAULT_{OPUS,SONNET,HAIKU}_MODEL` in the repo's own **shared, versioned `.claude/settings.json`** — never a personal `~/.claude/settings.json`, which only ever affects that one person's sessions and would let the same role run on different models for different teammates. When a run genuinely needs to be reproducible — an eval, an A/B — pin the model for that run's duration by setting the env var in the project file (a one-line PR, reviewed like any other parameter change), not by editing a role file; the token-report still records the actual model, read from the transcript (`message.model`), whichever way the alias resolved that day. Outside such a run, ride forward with the alias: a newer model is meant to replace the older one by default.
...
Each worker is spawned as **its own agent type** — `explorer`, `be-dev`, `fe-dev`, `designer`, `tester`, `checker`, `qa`, and `team-lead` for a sub-lead. Not `general-purpose` with the role definition pasted in. A role file is not a prompt template: it carries the role's default model and effort, the tool allow-list, and the name your run log and the hooks see. A generic agent given the same prose has none of those.

What you lose without noticing: the routing never happens (a generic agent runs on the session's model at the session's effort, so `sonnet · high` is never consulted); the tool restrictions never apply (`checker` is read-only because its definition says so, and a stand-in can write); and the no-worker-can-spawn invariant breaks, because that holds only by those roles omitting `Agent` from `tools`. Configuration you bypass is not enforcement.
...
Prevention beats the chase, and the prevention is the deliverable, not the wording: **for work a gate will grade — `checker`, `qa`, `tester` — name a FILE in the brief** — path plus "no file = task not done" — and read it from disk, because their verdict cannot be reconstructed from disk any other way. Same session: four message-deliverable briefs → six empty returns; one file-deliverable brief → a gradable artifact first try. **Name the moment as well as the path: the file is created with the worker's first change and appended to as it goes.** A file promised at the end is still a report held in memory. Measured 2026-08-30: one assignment died with its process holding an unwritten report, and the second attempt at the same task survived only because it wrote incrementally. Implementer roles (`be-dev`, `fe-dev`) do not get this file: their report is a message in fixed fields, capped at 40 lines, with narrative in the commit and declaration in `.claude/status/` (1.34.0 P5).
...
- **Silent returns are the failure mode that multiplies in both modes.** Fan-out is what multiplies it, and the prevention is unchanged: every brief names its deliverable — a report FILE written from the first change for a gate role (`checker`, `qa`, `tester`), and for an implementer the fixed-field message plus its commit and its `.claude/status/` declaration — and "released" is recorded only for a termination you actually observed. Reconstruct the live set from the run log, not from memory.
