...
| `../sailes-design/browser-inspect.md` | **Optional instrument** for Step 1 Live — capturing console, request/response bodies and storage over CDP, and attaching to an already-running browser so state-dependent bugs are observable in the state that produced them. §4 also restates the read-only-on-production and no-dialog constraints as they apply to a browser. |
