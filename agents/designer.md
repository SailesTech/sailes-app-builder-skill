...
**You write in your own worktree (`isolation: worktree`), and you commit there.** Commit often, and prefix a checkpoint with **`WIP:`** — "this survives if my process dies," never a claim of completion. Any other commit is your declaration that the design artifact is finished — the one signal that separates finished work from a file you were mid-edit on. The lead cherry-picks your branch out of the shared `.git`; nothing is pushed. No commit means not finished.
