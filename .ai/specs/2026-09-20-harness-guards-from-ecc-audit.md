...
Status: in-progress — **P1–P4 zamknięte 2026-09-20** (integracja `a4ea94e` na `feat/1.36.0-harness-guards`; evidence: `npm test` → exit 0, **25 zestawów** (23→25, dokładnie „+2" ze specu), 0 `not ok` · `node codex-agents/parity.test.js` → exit 0, koncepty pozytywne i **odwrotne** dla `checker`, `be-dev`, `fe-dev`, każdy sprawdzony w obie strony na kopiach · `node hooks/block-no-verify.test.js` → 16/16 · `node hooks/toolchain-guard.test.js` → 27/27 · sondy lidera payloadem z polem `cwd`, tak jak wysyła go Claude Code: `--no-verify` → exit 2, `core.hooksPath=` → exit 2, literał w komunikacie commita → exit 0, `tsconfig.json` w repo Sailes → exit 2, `package.json` → exit 0, **obce repo git bez `AGENTS.md` i bez `.ai/` → exit 0 cicho**, to samo repo po dodaniu `.ai/` → exit 2, `SAILES_TOOLCHAIN_GUARD=off` → exit 0 · żaden hook nie pisze na stdout przy blokadzie i żaden nie używa `permissionDecision` · **tester: done, 0 defektów**, plan `DERIVED`, 6 nowych ID z dowodem detekcji przez mutację ścieżki kodu · **checker: NITS** — obie sekcje obowiązkowe zamknięte formułą `reviewed against <surface>, none`, czyli klauzula A1 zadziałała na bramce, która ją wprowadza; jeden NITS (18 ID bazowych odwołanych numerem linii zamiast w nazwie testu) przyjęty przez lidera jako jawny i sprawdzalny · `qa: n/a — brak działającej aplikacji`). Workflow `wf_238e2c36-4f6`, 6 agentów, 0 błędów. Run log: `.ai/runs/2026-09-20-harness-guards.md`. **Human-STOP P4 rozstrzygnięty 2026-09-20:** człowiek zatwierdził listę chronionych ścieżek (ESLint · Prettier · Biome · Ruff · `tsconfig*`, bez `package.json`) i nazwę furtki `SAILES_TOOLCHAIN_GUARD=off` — obie zgodne z tym, co poszło w briefach, więc fala 2 nie wymagała powtórki. **Zostało: P5 (evale — 3 nowe + 8 re-runów) i P6 (wydanie).** Open Questions Q1–Q5 zamknięte 2026-09-20; approved 2026-09-20 przez właściciela. Pre-implement: READY-WITH-FIXES (`.ai/audits/2026-09-20-pre-implement-harness-guards.md`); R1, R2, R3, R5, R6 wpisane, P5 rozbite na P5+P6; R4 rozstrzygnięty — patrz P3.
