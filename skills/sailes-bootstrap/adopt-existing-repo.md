...
- **Harness guardrails, both twins:** `.claude/settings.json` + `.codex/config.toml` (from `codex-config-template.md`) sharing `.claude/hooks/*.sh`, plus `.github/copilot-instructions.md`. Wire the hook guards to the repo's REAL protected surface (their migrations dir, their prod-deploy command). Additive — if the repo already has one twin, add the missing one; never overwrite.
  - **The consequence of "never overwrite", stated so nobody assumes otherwise:** an adopted repo keeps its OWN copies of `.claude/hooks/*.sh`. The plugin auto-updates `skills/`, `agents/` and the framework's own hooks on every machine — it does **not** touch a repo's hook scripts. So framework changes to `hooks-template/*.sh` (the STATE.md snapshot check, the `ENV-LOCK` runtime guard) reach an existing repo **only when a human deliberately re-copies the script.** In Upgrade mode, show the human the diff between their `.claude/hooks/*.sh` and the current template and let them decide — silently leaving them on a version that predates a guard is exactly the drift this mode exists to catch.
...
- `.ai/business-logic.md` from `business-logic-template.md` — and on a brownfield repo, **seed it rather than leaving it empty.** The material is already on disk: `.ai/incidents/` (a resolved incident usually contains the ruling that resolved it), `.ai/STATE.md` (owner rulings buried in session memory — measured 2026-08-22: one such section sat at line 168 of a 197 KB file), any `dokumentacja/`, `docs/` or `*-flows.md` describing the domain, and the code's own constants. Extract only what passes the admission test (*does it stay true after the current work ships?*), stamp each with its source, and **mark as `UNKNOWN` what you could not attribute** — an unsourced rule is a guess wearing a citation. Then show the human the list and ask them to confirm or correct it; their corrections are the most valuable rows in the file.
  - **Do NOT migrate an existing `logika_biznesowa.md` / `business-logic.md` by copying it.** Measured 2026-08-22: such a file had rotted into a body, a contradicting banner, and an index still advertising the retracted claim. Treat it as a *source to mine*, like any other doc, and let the admission test decide what survives.
