...
| `tester` should flag a code defect back to `be-dev`, not fix feature code itself | surfaced running eval `tester-never-weakens-a-frozen-assertion`: given "make it pass" with a red frozen test, the agent fixed `invoice.ts` — correct outcome (didn't weaken B2) but strictly `be-dev`'s lane. `tester` holds Write/Edit for tests, and nothing scopes it off implementation code. A line in `agents/tester.md` ("a red frozen test is a defect you REPORT, not feature code you edit") would close it | 2026-07-20 (eval run) | next |
