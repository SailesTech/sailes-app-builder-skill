...
## Pre-flight
1. Read the spec fully + its **phases/steps** + the pre-implement readiness report. Start from the SessionStart hook's excerpt of **`.ai/STATE.md`** (Open failures / General rules / Last session, or the file head when those sections are absent) — don't re-derive what's already verified or known to fail. Before non-trivial work in a specific area, `grep` **`.ai/lessons.md`** and **`.ai/archive/`** by that area's keywords (module, file, integration name); neither file is read whole "before any task".
2. Confirm `Status: approved`; set it to `in-progress`.
3. For long/multi-step work (>~5 commits), open a **run log** `.ai/runs/{YYYY-MM-DD}-{slug}.md`: goal, phase list, decisions, what's left, and a `Known-red:` section — one entry per test that was red on the base before this push, form `<file/test name> · <cause> · validity: this push` (never a count; `agents/qa.md` populates it at the pre-push gate, per F3) — so the work is resumable across context resets. **Its critical-path section carries two drawings, not one:** the graph of phases *and* the file-ownership matrix. An arrow in a phase graph records the order somebody thought about the phases in, not a technical dependency — measured 2026-08-01, a plan called a phase "solitary" twenty lines above its own table showing that phase's files were disjoint from the next one's, and it idled behind six others for nothing. Dispatch on set intersection, never on arrows (`agent-team-structure.md`, rule 2). The file-ownership matrix is a fenced ```yaml block, not prose, so it can be compared instead of read:
   ```yaml
   ownership:
     F1:
       - path/one
...
1. **Plan the step** — restate what it changes and the check that will prove it. Identify the RED test first (write or name a failing test before the code — `superpowers:test-driven-development`).
   - **This is your inner loop — an instrument for cycle time, not a ritual.** Seconds to run, run constantly, as many as the step needs; implementation-shaped by design; **no ID, no freeze, no detection proof**; deleted or rewritten freely; **never gate evidence**. The graded suite is authored later by `tester` (`sailes-test`) from the spec with your implementation unread, so it cannot mirror the code. Until 2026-08-30 the only word for this was "scaffolding", which made every quick check look like it owed the gate's ceremony — so it got skipped or over-formalised, and both cost the step.
   - **What runs in the loop is scoped, not "whatever's fastest to type."** Only the tests for the files this step touched — the phase gate itself (step 4/`Done-when`) is the phase's own **named, targeted commands**, never the full suite. The full suite and e2e run **once, after the last phase, before push**, by `qa`, on the integrated branch, holding the environment exclusively — this replaces the 1.33.0 rule that ran them per worker, once before the declaration commit (`agents/qa.md`; `skills/sailes-bootstrap/release-checklist.md`). Measured 2026-09-12: two of the most expensive worker runs re-ran a full `yarn test` seven times and a full `test:e2e` seven times each, inside briefs that never said the inner loop should stay narrower than the release gate — the brief's `Verification:` line (`agent-team-structure.md`) is where this split is now stated explicitly, and it is the reason the inner loop stays narrow rather than the reason it grows a second full run per phase.
   - **Report the one that caught something, under a fixed label.** A check that went red on a **real defect** — not merely on code you had not written yet — is the only test here whose detection was *earned rather than argued*. Write `Promotion candidate:` in your report with the check's exact name and the defect it caught; `tester` folds it in at step 4. The label is fixed so the handoff is greppable rather than inferred: measured 2026-08-30, an arm that had the rule wrote an excellent section about the defect it found and titled it something no downstream role would search for. You never ID it or add it to the frozen list yourself — that door stays the gate's. `tester` may supersede a scaffolding test only with an ID-bearing equivalent, never delete one to reach green.
...
4. **Verify (behavior before diff)** — drive the real running system first (e2e flow / `curl` the live endpoint / click the UI / generate the actual PDF/screen), observe the real behavior, THEN trust it. Paste the evidence (command + output / screenshot). A green build/lint is not proof; "looks done" is the failure mode. **UI-touching steps get vision-verify, in the `full` lane:** compare the fresh screenshot against the design artifact and the previous accepted screenshot in `.ai/screens/` (canon: `sailes-bootstrap/agent-team-structure.md`, Gate isolation). In the `middle` lane, evidence is the live-run output, no screenshot required.
...
6. **Track** — tick the step in the spec's **Progress** section (and the run log, where Pre-flight opened one). New unknown surfaced → stop, resolve via `sailes-spec` (re-gate), don't guess. At each **phase** gate, also update the root `STATUS.md` (client-readable: phases done/total, the plain-language Done-when result, accepted screenshot for UI phases **in the `full` lane** — never effort/pricing data).

**Phase gate (binary stop condition).** A phase is complete only when its **Done-when** condition from the spec passes — run the exact commands, paste the output. **Where the phase carries a `Deployed-probe:`, that command runs too, against the deployed host** — a `Done-when` green against origin, `localhost` or a mock says nothing about the wire property the phase is keyed on, because a CDN can rewrite the origin's status and `Content-Type` before any customer sees them. "Looks complete" is not a phase gate. If the spec has no binary Done-when for a phase, derive one and add it to the spec **before** starting that phase. A Done-when never overrides decision ownership: hitting a **key decision** mid-loop (contract shape, data model, auth, a new UX surface) means STOP and escalate per `agent-team-structure.md` — never push through it to satisfy the goal. Once the gate closes, the lead does not carry straight on into the next phase on the same context — see the `session-handoff` rule in `agent-team-structure.md` for what closes the turn.

**Pre-push gate (once, after the last phase — never per phase).** Every phase gate above is the phase's own named, targeted `Done-when` commands, run by whoever built the phase — `be-dev`/`fe-dev` never run the full suite or e2e on a phase, and neither does `checker`. After the last phase closes, `qa` runs the full test suite and the e2e requirement **once**, on the integrated branch, holding the environment exclusively for the run (`agents/qa.md`; `skills/sailes-bootstrap/release-checklist.md`). This is the gate before push, not a repeat of the phase gate at a larger scope — it exists because no single phase's `Done-when` proves the whole branch still holds together. **When that run has red tests, pre-existing red is established by name, never by count** (`agents/qa.md` carries the exact procedure: paste the red names on the branch, run those same names at the merge-base, paste `comm -23`; a non-empty result is CHANGES-REQUIRED, and names red on the base go to the run log's `Known-red:`).

## Test → Review → Behavior gate (before "done")
- **Test authoring in a fresh context** — `tester` (`sailes-test`) derives the phase's expected behavior from the spec *with the implementation unread*, the human freezes the case list to `.ai/test-plans/<spec>.md`, then `tester` writes the suite and proves it detects at the feature's risk tier. This runs **per phase**, after the code and before `checker`. The RED test the dev named in step 1 is scaffolding for that step; the `tester` suite is the graded artifact, authored under isolation so it cannot mirror the code. **In the `middle` lane** (the phase's `Lane:` line, tier B/C) the plan goes straight to `Status: DERIVED` and `tester` writes immediately — no human freeze STOP.
...
- **`Workflow` is the default path once a spec has more than one phase to dispatch.** Running each phase as a separate `Agent` call costs a lead turn per dispatch and a lead read per return; a `Workflow` script drives the phases, gates and STOPs in one call and returns schemas the lead writes to disk instead of transcripts the lead has to read. Pipeline shape (WF1/WF2/WF3, lane middle folding WF1+WF2), the brief every phase prompt carries, the three result schemas, the `safe()` wrapper, `null`/`blocked` handling, integration, model resolution and cost measurement are all in `sailes-bootstrap/workflow-orchestration.md` — that file is the single source, not restated here.

## Quick Reference

| Stage | Gate |
|---|---|
| Pre-flight | spec approved + READY; STATE.md hook excerpt read, lessons.md + archive grepped by area; status→in-progress; run log (>~5 commits); branch |
| Per step | RED test → implement → test → verify (evidence) → commit → track |
| Per phase | **Done-when passes** — exact commands run, output pasted |
| Test (per phase) | `tester` (`sailes-test`): cases from spec with code unread → human freezes `.ai/test-plans/<spec>.md` (`middle` lane: `DERIVED`, no freeze STOP) → write suite → tiered detection proof |
| Review | adversarial fresh-context review vs spec + checklist (checker sees diff + rubric only; every frozen behavior ID covered) |
| Pre-push (once, after the last phase) | `qa` runs the full suite + e2e on the integrated branch, exclusive environment |
...
- The phase depends on a status code, header or `Content-Type` and you proved it against origin, `localhost` or a mock. Those three are the surfaces that were all green while the deployed one was not.
