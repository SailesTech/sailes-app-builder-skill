...
- **Fact-finding** (about *their* world): "Which CRM?", "How many users?", "What is already running?" → plain options, no recommendation needed; you are learning their reality.
- **Decision** (a fork *you* would otherwise pick): stack, ORM, auth, hosting, tenancy, integration depth, build-vs-buy, roles. → **NEVER** a bare "A or B?" checkbox, and never decided silently.

**The decision-card format, its quality bar, and what to do when you cannot ground a recommendation live in `decision-card.md`.** Read it when you hit a real fork. It covers the case the format used to have no answer for: a fork you cannot honestly ground, where "nie mam podstaw, żeby wskazać" is the correct recommendation line and settling it by measurement is the fourth option next to A/B/C.

### The checklist for this variant

**`checklists.md`** holds both — greenfield (Project Brief) and brownfield (Task Brief). Walk the one Step 0 selected; you never need both in one session. Skip an item only if the user explicitly defers it.
