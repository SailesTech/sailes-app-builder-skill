...
Prevention beats the chase, and the prevention is the deliverable, not the wording: **for work a gate will grade, name a FILE in the brief** — path plus "no file = task not done" — and read it from disk. Same session: four message-deliverable briefs → six empty returns; one file-deliverable brief → a gradable artifact first try. **Name the moment as well as the path: the file is created with the worker's first change and appended to as it goes.** A file promised at the end is still a report held in memory. Measured 2026-08-30: one assignment died with its process holding an unwritten report, and the second attempt at the same task survived only because it wrote incrementally.
