...
- `npm test` — fifteen suites: hook tests (`hooks/*.test.js`), the four **governance tools**
  ...
  this against `package.json` rather than against this sentence — **it was wrong twice on
  2026-08-02, in both directions.** First it omitted the `tools/` suites that had run in the gate
  since 1.27.0 (found by a `docs-author` reconciling the architecture diagram against
  `package.json`). Then the fix asserted `mcp-toolnames-check.test.js` was among them when nothing
  ran it — an enforcement claimed and absent, which is the failure this repo already records for
  `agents-md-template.md`'s lint rule, reproduced here by the person correcting the first error.
  Caught by the worker that had just written the file. It is wired now; the count above is the
  check.
