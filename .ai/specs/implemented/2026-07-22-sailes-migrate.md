...
| D7 | **Branch base + version number** | (A) branch off `main` (1.11.0) → bump 1.12.0 (collides with graphify); (B) branch off current `feat/graphify-default-integration` (1.12.0) → bump **1.13.0** | **(B) 1.13.0 on top of graphify** | Medium regret. If graphify is NOT going to `main`, rebase this branch onto `main` and renumber. Chosen for file+version consistency with the tree I read. |
...
- `CHANGELOG.md` — a `1.13.0` entry, upgrade-actionable.
- Version bump **1.12.0 → 1.13.0** across all four manifests: `VERSION`, `package.json`,
...
- C1. Add the rows to `skills/README.md`, `README.md`, `AGENTS.md`; add the `CHANGELOG.md`
  1.13.0 entry; bump all four manifests to 1.13.0. **Done-when:**
