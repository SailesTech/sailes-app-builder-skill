...
| **sailes-discovery** | The interview before the spec. Pulls full intent into a structured Brief. **Must** chain into bootstrap on greenfield (never stop at the spec). | `SKILL.md` (thin entry), `decision-card.md` (the fork format + what to do when you cannot ground a recommendation), `checklists.md` (greenfield + brownfield — you need one, never both), `brief-template.md` |
...
| **sailes-test** | The test suite for an implemented phase — the last verification step before `checker` and `qa`. Derives expected behavior from the **spec, before reading the implementation**, freezes a case list with the human, then writes tests that detect faults instead of mirroring the code. Distinct from `sailes-eval-runner`, which grades *model behavior* against an `evals/` scenario and shares its "przetestuj" trigger stem: a test is deterministic, an eval is not, and they are never interchangeable. | `SKILL.md` |
| **sailes-docs** | The documentation layer of every Sailes repo: the archify diagram set under `docs/architecture/` (JSON + HTML committed, HTML `.claudeignore`d), authored from repo evidence and held to a `validate`/`deliver` receipt. Gated, not conventional — the delta step runs at **every** spec closure before the `git mv`, and an explicitly empty delta is evidence ("this spec changed no architecture"), never a skipped step. Machine prereq `npx skills add tt-a1i/archify -g` (floor >= 2.12) with an explicit-SKIP protocol; built on [archify](https://github.com/tt-a1i/archify) (MIT). | `SKILL.md`, `references/archify-setup.md`, `references/authoring.md`, `references/delta-at-gate.md` |
...
| **sailes-eval-runner** | **Framework maintenance, not client-app work** (and a skill rather than a role precisely so it costs nothing to the majority who never run it): dispatch an `evals/` scenario faithfully to a fresh context, grade the **artifact** against the scenario's own binary criterion, and record a verdict that states what it covers. Carries the two distinctions the framework's own author kept losing — **stand-in vs named role** (a stand-in grades the text, never the runtime) and **blocked vs run** — plus the rule that a fixture must *create* its condition, not describe it. | `SKILL.md` |
...
8. **Memory compounds, or it decays.** `.ai/STATE.md` (verified facts / open failures / last session) is delivered as its current excerpt by the SessionStart hook, and written before walking away — nobody reads it or `.ai/lessons.md` whole "before any task"; instead, `grep` both plus `.ai/archive/` by the touched area's keywords. Both files stay under a size limit (20 / 40 KB) and rotate older entries to `.ai/archive/` verbatim. Every spec phase carries a binary `Done-when`; gates see the artifact + rubric, never the maker's narrative; recurring lessons get promoted (preferably as an enforced check, else AGENTS.md/Task Router; cross-project pattern → skill candidate).
