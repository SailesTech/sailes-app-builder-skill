...
Your worktree is cut from `main`, which lacks P1–P3. Run `git merge --ff-only 738be36` before any edit; HEAD must then be `738be36`. If it is not a fast-forward or not that commit, STOP and report. Then claim `.claude/status/be-dev-P5ab-A5.md` before the first edit (other `be-dev-*` files there belong to other runs — do not touch them).
