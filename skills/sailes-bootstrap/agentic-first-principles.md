...
**The roster — who each role is, what it must never do, and the model and effort it defaults to — lives in exactly one place: `agent-team-structure.md`.** It is not restated here. That table used to exist in three files and all three had drifted by 2026-07-26; two had lost `tester` entirely, which is a missing gate that reads like a complete list. What follows are the *principles* that govern the team, which is what this file is for.

- **Order:** `explorer → designer → BE contract finalized → fe-dev → tester → checker → qa` — the `full` lane; a tier B/C phase runs the `middle` lane (the `gate-scaling` block in `agent-team-structure.md`). One task per worker; workers escalate scope questions to `team-lead`. **Every worker that WRITES runs in `isolation: worktree`** — the test is "does it write", not "is it listed" — and **never commits to a shared branch or pushes; inside its own worktree it commits, and should** (a commit is its declaration that the work is finished; the lead cherry-picks the branch out of the shared `.git`). Worktrees isolate **files, not the runtime environment** — the database, ports and containers are shared, which is why `qa` takes exclusivity instead.
...
- **Harness parity (not just docs):** the *instructions* transfer via AGENTS.md; the *guardrails* need a per-harness twin. Claude Code = `.claude/settings.json` (permissions + hooks); Codex CLI = `.codex/config.toml` (`sandbox_mode`/`approval_policy` + `[hooks]`). The hook **scripts are shared** — both harnesses use the same stdin-JSON payload + exit-2-to-block contract, so `.claude/hooks/*.sh` is one copy referenced by both configs (`codex-config-template.md`). Skills are portable too: the same `SKILL.md` (`name` + `description` frontmatter) is a Claude plugin skill AND a Codex skill under `.agents/skills/` — install once per harness, one source. A repo is "Codex-ready" only when the `.codex/` twin exists and points at the shared scripts, not merely because AGENTS.md is readable.
...
- **Isolation for parallel work — a worktree is the default for agents, not a fallback.** A human working two things at once uses a branch or a **git worktree** (`git worktree add ../wt-<name> <branch>`), one per concern. For **agents the worktree is mandatory whenever the agent writes** (`isolation: worktree`): two processes writing one file on a shared disk do not produce a conflict you can merge, they produce **silent loss** — git only ever sees the survivor. Add `.claude/worktrees/` to `.gitignore`; the branch in the shared `.git` is the artifact, the directory never is.
...
- **Lessons file (`.ai/lessons.md`).** After a correction or a recurring bug, append a lesson: **Context / Problem / Rule / Applies-to**. This is the repo's durable memory, kept ≤40 KB (rotate the oldest lessons to `.ai/archive/` verbatim, past that). Before non-trivial work in an area, `grep` `.ai/lessons.md` **and** `.ai/archive/` by that area's keywords (module, file, integration name) — not by `Applies-to`, which most entries lack, and never the whole file "before any task". AGENTS.md carries a rule: "after a correction, record a lesson here."
- **State file (`.ai/STATE.md`) — the project's session memory.** Five sections: **Verified facts** (each with the evidence that proved it) / **General rules** (distilled) / **Open failures** (unresolved + best diagnosis) / **Lessons learned** (pointers into lessons.md) / **Last session** (where work stopped + next step). Kept ≤20 KB; rotate closed Open failures and older Verified facts to `.ai/archive/` verbatim when near the limit. Two iron rules: the **SessionStart hook delivers only the current part** (Open failures / General rules / Last session, or the file head under the size limit when those sections are absent) — nobody reads the whole file "before any task" — and **write before walking away** (every session — completed or interrupted — ends with the update). Division of labor: lessons.md holds *portable rules*; STATE.md holds *this project's facts* and the resume pointer.
