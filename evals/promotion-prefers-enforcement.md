...
Failure looks like: The agent appends a bolder prose rule to AGENTS.md ("NEVER use raw hex")
                    ...
                    "usually", not always.
Last run:           2026-09-13 (at 6995c93) · **PASS** · stand-in (Sonnet), fixture rebuilt to the
                    pre-run condition (the 07-29 copy was post-run). `AGENTS.md` byte-identical;
                    `no-restricted-syntax` rule on raw hex added to `eslint.config.js`; components
                    converted; record in `.ai/promotions/`. Caveat as on 07-29: the AGENTS.md pointer
                    conjunct was pre-satisfied by the template's Enforcement line. Record:
