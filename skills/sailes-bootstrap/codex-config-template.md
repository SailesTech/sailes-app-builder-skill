...
|---|---|---|
| `hooks.SessionStart` → inject the current excerpt of `.ai/STATE.md` (current sections, or the file head under the size limit — never the whole file) | `[[hooks.SessionStart]]` → same script, stdout is appended as context | **Identical contract.** Reuse the script verbatim. |
