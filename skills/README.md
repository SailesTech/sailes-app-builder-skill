...
| **sailes-hosting** | Hosting/ops reference (not part of the core pipeline): how we host & deploy on **Railway** — project/env/service topology, all env vars & secrets, Postgres/S3-bucket/Volume storage layers, ephemeral-FS rule, git-branch deploy (incl. flattened build-branch caveat), `start:prod` migrate-on-boot, `/health`, `railway logs` debugging, install/OAuth-callback gotchas. Complements `sailes-bootstrap/release-checklist.md` (the release gate). Real Sailes patterns. | `SKILL.md`, `references/*` |
